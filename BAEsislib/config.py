...
from .so3.normalization import Normalization
...
class Config:
    ...
    @staticmethod
    def set_normalization(normalization: Normalization = "4pi") -> None:
    ...
    def set_tensor_conversion_cache(
        ...
        Args:
          path: Path to disk cache for saving/loading tensor conversion
            coefficients.
        """
        Config.tensor_conversion_cache = epath.Path(path) if path is not None else None
